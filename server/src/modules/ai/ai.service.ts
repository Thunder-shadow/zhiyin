...
${isFirst ? '这是面试开始，请先做简短的自我介绍引入，然后请求职者做自我介绍。' : ''}`;
  ...
  /** HR反向模拟 - 流式 */
  private async *handleHrSimStream(body: { conversation?: any[]; resume_index?: number; end?: boolean; candidate?: any; [key: string]: any }, headers: Record<string, string>): AsyncGenerator<StreamChunk> {
    const config = new Config();
    const customHeaders = HeaderUtils.extractForwardHeaders(headers);
    const client = new LLMClient(config, customHeaders);
...
${isFirst ? '这是面试开始，请先做简短开场，然后请求职者做自我介绍。' : ''}`;
  ...
  /** HR反向模拟 */
  private async handleHrSim(body: { conversation?: any[]; resume_index?: number; end?: boolean; candidate?: any; [key: string]: any }, headers: Record<string, string>) {
    const config = new Config();
    const customHeaders = HeaderUtils.extractForwardHeaders(headers);
    const client = new LLMClient(config, customHeaders);
