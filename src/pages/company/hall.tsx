...
import { Building, Plus, ChevronRight } from 'lucide-react-taro'
  ...
  return (
    <View className="min-h-full bg-background px-4 pt-4">
      ...
      <View className="flex flex-row items-center justify-between mb-4">
        <View>
          <Text className="block text-lg font-bold text-foreground">副本大厅</Text>
          <Text className="block text-xs text-gray-400 mt-1">攻略心仪公司，拿下Offer</Text>
        </View>
        <Button
          size="sm"
          className="bg-accent text-white border-none rounded-lg"
      ...
      <View className="flex flex-row gap-2 mb-4 overflow-x-auto">
        <View
          className={`px-3 py-1 rounded-full ${filter === 'all' ? 'bg-primary' : 'bg-white'}`}
        ...
        {Object.entries(statusMap).map(([key, val]) => (
          <View
            key={key}
            className={`px-3 py-1 rounded-full ${filter === key ? 'bg-primary' : 'bg-white'}`}
      ...
      {filteredCards.length === 0 ? (
        ...
        <View className="flex flex-col gap-3 pb-4">
          {filteredCards.map((card) => {
            ...
            return (
              <Card key={card.id} className="shadow-sm" onClick={() => Taro.navigateTo({ url: `/pages/company/detail?id=${card.id}` })}>
                <CardContent className="p-4">
                  <View className="flex flex-row items-center justify-between">
                    <View className="flex flex-row items-center gap-3 flex-1">
                      <View className="w-10 h-10 rounded-lg bg-primary flex items-center justify-center flex-shrink-0">
